...
#            Extension: Put these plots in tabs so the app looks tidier.
...
ui <- fluidPage(    # create user interface using fluid-page function
  ...
  sidebarLayout(    # indicates layout is going to be a sidebar-layout
    ...
    sidebarPanel( # open sidebar panel
      ...
      selectInput(inputId = "SI_n_sim",                  # id of input, used in server
                  label = "PSA runs",                    # label next to numeric input
                  choices = c(10,100,1000,10000,100000), # choices 
                  selected = 1000),                      # default choice
...
server <- function(input, output, session){   # server = function with two inputs
  ...
  # this bit of code is an example of dynamic UI - it updates one part of the UK based upon another.
  observeEvent(input$SI_n_sim, {
    
    label <- paste0("Run ", as.numeric(input$SI_n_sim), " PSA runs")
    
    updateActionButton(input = "run_model", 
                       label = label,
                       session = session)
  ...
  observeEvent(input$run_model,       # when action button pressed ...
               ignoreNULL = F, {
                 ...
                 tooBig <- as.numeric(input$SI_n_sim) > 10000
                 ...
                 df_model_res = f_wrapper(c_Trt      = input$SI_c_Trt,
                                          ...
                                          n_sim      = as.numeric(input$SI_n_sim))
