...
#========================================================================#
# TIME VARIANT MARKOV MODEL #

m_TR_tv     <- m_TR[] * NA
m_TR_tv[1,] <- c(1,0,0,0)

# now have a three dimensional array (extra dimension being time).
# it is identical though

a_P <- array(data = m_P, 
             dim = c(dim(m_P), n_t), 
             dimnames = list(dimnames(m_P)[[1]],
                             dimnames(m_P)[[2]],
                             paste0("cycle",1:n_t)))

for (t in 1:n_t){                              # throughout the number of cycles
  
  m_TR_tv[t + 1, ] <- m_TR_tv[t, ] %*% a_P[ , , t]          # estimate the Markov trace for cycle the next cycle (t + 1)

  }

# how many don't match??
sum(m_TR != m_TR_tv)    # now we know we match we could change the transmission matrices in the array.


# simple example
# what if everyone dies in cycle 4???
a_P[ , ,4] <- 0
a_P[ , 4, 4] <- 1
a_P[ , ,]
...
plot(m_TR[,"D"], col = "black", ylim = c(0,1), type = "l")
lines(m_TR[,"S1"],col = "red")
lines(m_TR[,"S2"],col = "orange")
lines(m_TR[,"H"], col = "green")

plot(m_TR_tv[,"D"], col = "black", ylim = c(0,1), type = "l")
lines(m_TR_tv[,"S1"],col = "red")
