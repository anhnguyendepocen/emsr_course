...
f_wrapper <- function(
  ...
  u_Trt   = rtruncnorm(1000, mean = 0.95, sd = 0.02, b = 1),
  show_progress_bar = T
  ){
...
gen_psa <- function(n_sim = 1000){
  ...
  df_psa <- data.frame(
    ...
    u_Trt = sample(x = u_Trt,size = n_sim,replace = T)  # utility when being treated
  )
...
if(show_progress_bar){
  progress1 <- shiny::Progress$new(min = 0,max = n_sim,
                                   style = getShinyOption("progress.style", default = "notification"))
  progress1$set(message = "Running model", value = n_sim)
...
for(i in 1:n_sim){
  if(show_progress_bar){
    progress1$set(value = i, 
                  detail = paste("PSA iteration ", round((i/n_sim)*100 ),"%",sep=""))
  }
